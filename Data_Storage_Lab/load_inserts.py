...
DBname = "postgres"
DBuser = "postgres"
DBpwd = "1234"   # insert your postgres db password here
...
def createTableWithoutConstraints(conn):
    with conn.cursor() as cursor:
        ...
        """)
    print(f"Created {TableName} without constraints/indexes")
...
def load_with_copy_from(conn, fname):
    # slide 8
    with conn.cursor() as cur:
        print(f"Loading data via copy {fname}")
        with open(fname,'r') as f:
            next(f)
            cur.copy_from(f, TableName, sep=',', null='')
    conn.commit()
    print("COPY load complete")

def main():
    initialize()
    conn = dbconnect()
    #if CreateDB:
    # createTableWithoutConstraints(conn)
